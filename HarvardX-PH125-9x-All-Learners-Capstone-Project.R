...
# The number of reviews for each movie varies extremely, with most movies receiving only 
...
# Exploring adding a penalty term in the mean movie ratings to reduce the deviation by rarely  
# seen movies. The weighted mean ratings with an added penalty coefficient lambda for a movie is  
# given by dividing the sum of ratings by the sum of lambda plus the number of reviews for that movie:
#
...
# the training set into 10 equal subsets with i containing the corresponding indexes:
...
# is computed. The average MSE across all 10 subsets are in turn computed. Finally, lambda values 
# between 0 and 4 are fed into the function to see which one gives the least RSME.
...
# With this, the RMSE in the test set is re-evaluated. Note mu is to be subtracted from the ratings  
# in the test set first.
...
# The movie averages are now subtracted from the training set ratings to further analyze the residual errors. 
...
# With this the RMSE in the test set is re-evaluated. Note both mu and movie mean are subtracted from the 
...
# preference of users instead of just estimating with a single user average.
...
# The next step is to separate the genres column into a maximum of 8 different columns, gather each under a 
# row and group the genres. The empty columns and the genre with the entry "(no genres listed)" are removed. 
# A total of 19 genres are present. 
...
# A user genre table (user_gen) is created in a similar manner to mov_gen, but it computes the average rating of the
...
# any of the genres of the movie in question, his/her overall rating (user_avg) will be used instead. This improves the 
...
# The average residual ratings for each movie and the no. of reviews per movie are saved in table rsd_mov_avg. 
...
# A regression tree model is created to predict the residual ratings by partitioning the movies by review count. 
...
# The residual rating estimates are plugged back into the table rsd_mov_avg via the predict function. 
# Then include this term along with the previous variables for subtraction in the final RMSE evaluation.
